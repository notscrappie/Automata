...
export class Manager extends EventEmitter {
  ...
  public create(options: ConnectionOptions): Player {
    ...
    const foundNode = this.nodes.get(
      options.region
        ? this.leastUsedNodes.find((node) =>
            node.regions.includes(options.region.toLowerCase())
          )?.options.name
  ...
  public async resolve(
    ...
    if (!this.isActivated)
      throw new Error(
        "Automata has not been initialized. Initiate Automata using the <Manager>.init() function in your ready.js."
      );
    ...
    const regex = /^https?:\/\//;
    const identifier = regex.test(query)
      ? query
      : `${source ?? "dzsearch"}:${query}`;

    const res = (await node.rest.get(
      `/v3/loadtracks?identifier=${encodeURIComponent(identifier)}`
    )) as LavalinkResponse;
    const mappedTracks =
      res.tracks.map(
        (track: TrackData) => new AutomataTrack(track, requester)
      ) || [];
